...
def currentOrderList(request):
	# try:
	orders = OrderList.objects.all()			#use get to remove isfinished
	orderNumbers = []
	orderObjectList = []
...
def showHistory(request):
	...
	print(context['orderItems'][0])
	return render(request,"history.html",context)
	# except:
		# return render(request,"backedUpOrders.html",{})

@csrf_exempt
def orderCompleted(request):
	orderNo = request.POST['orderNum']
	print(orderNo)
	orderToBeDeleted = OrderList.objects.get(orderNum = orderNo)
